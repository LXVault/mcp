...
description: Entry point for the mcp-rag-mcp-server. Resolves the shared instruction set, declares the tools that fire, and routes to the local index.
---

# AGENTS

This repository is `mcp-rag-mcp-server`, a Model Context Protocol server that exposes the
MCP RAG knowledge base to MCP compatible clients such as Claude Desktop and IDE
assistants. It speaks JSON-RPC over stdio and holds no state and no logic of its own:
every tool is a thin call to the Express backend in `LXVault/server-expressjs`, carrying a
per project access token so the backend can attribute each action to the user who issued
that token.
...
|---|---|
| Add or change a tool, its description, or a backend call | [`.agents/rules/repository.md`](.agents/rules/repository.md) |
...
* `AGENTS.md` and `README.md` are overviews and must never carry detailed rules or
  documentation. The tool reference, the packaging guide, and the environment variables
  are documented in `wiki/`, not here.
...
* **Local carries only what is local.** A convention true for more than this server
