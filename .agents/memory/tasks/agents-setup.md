...
description: Record of the agent instruction, knowledge and memory system setup for this MCP server. Goal, mode, what was created, and the decisions taken.
---
...
|---|---|---|---|---|---|
| 1 | Agent instruction system | Entry point, indexes, local rules, both wiki trees, memory seed, first log | mcp | `docs/agents-setup` | 6 |
...
* `.agents/rules/repository.md`, the local rules: tools stay thin, authorization lives in
  the backend, tool descriptions are the interface, stdout belongs to the transport, and
...
* `wiki/information/overview.md`, `wiki/information/architecture.md`,
  `wiki/reference/tools.md`, `wiki/environments/setup.md`, `wiki/environments/env.md`,
  `wiki/guides/claude-desktop-extension.md`.
* `wiki/logs/1/0/0/CHANGELOG.md`, the first log entry.
* `README.md` rewritten as an overview. Nothing was dropped: the tool table and its
  prerequisites moved to `wiki/reference/tools.md`, the bundle packaging procedure moved
  to `wiki/guides/claude-desktop-extension.md`, and the variable table moved to
...
* **Two extra wiki folders.** `wiki/reference/` and `wiki/guides/` were created because the
  existing README carried a tool reference and a packaging procedure, and the setup
  procedure requires detail to be relocated rather than deleted.
